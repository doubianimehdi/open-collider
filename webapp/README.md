...
**User manual (French):** [`static/manuel.md`](static/manuel.md) — also at `#/manuel` in the running app.  
**Quick guide (English):** `#/guide` in the top bar.

**En clair:** each idea card includes a French actionable summary (headline, link to your brief, three concrete actions, one 30-minute test) generated at idea creation — LLM in live mode, simulated templates in demo.
...
3. **Curate** — the top retained ideas are presented as cards with per-axis score
   bars, the **En clair** block (what to do in plain French), and collision context.
   Flag them love / like / trash and add a steering note.
...
5. **Synthesis (Bilan)** — read favorites, validate retained ideas, get an aggregated
   REPORT.md (and REPORT.html). Per-iteration ITER_REPORT.html is generated when
   you lock in curation feedback.

...
from open_collider.report_html import write_brainstorm_html, write_iteration_html
import json
import logging
import re
import uuid
...
def generate_iter_html_report(project_dir: str, iteration: int) -> str:
    """Generate iter_NNN/ITER_REPORT.html — clear synthesis-style export."""
    project_path = Path(project_dir)
    bs_state = _load_state(project_path)
    brainstorm_dir = project_path / "brainstorms" / bs_state.get("brainstorm_id", "")
    return write_iteration_html(project_path, brainstorm_dir, iteration)
...
def generate_brainstorm_report(project_dir: str) -> str:
    ...
    write_brainstorm_html(project_path, brainstorm_dir, iter_summaries)
